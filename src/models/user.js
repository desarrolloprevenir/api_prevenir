...
userModel.registerUsu = (usu, callback) => {
    if (connection) {
        ...
        console.log(id, usu);
        var sql = 'INSERT INTO usuarios(id,cedula,correo,avatar,nombre,apellidos,members_id,parentescos_id_parentescos,telefono) VALUES (?,?,?,?,?,?,?,?,?)';


        connection.query(sql, [id, cedula, correo, avatars, nombre, apellido, id, usu.parent, usu.tel], (err, row) => {
            if (err) {
                connection.query('DELETE FROM members WHERE id = ?', [id], (err, res) => {
...
userModel.setUsuario = (usu, callback) => {
    if (connection) {
        ...
        connection.query(sql, [usu.tipoDocumento, usu.cedula, usu.nombre, usu.apellidos, usu.direccion, usu.telefono, usu.telefonowatshapp, usu.fecha_nacimiento, usu.estadoCivil, usu.ocupacion, usu.barrio, usu.eps, usu.id], (err, row) => {
            if (err) { callback(null, { 'update': false }); } else {
...
userModel.darUsuario = (ced, callback) => {
    if (connection) {
        ...
        connection.query(sql, ced, (err, row) => {
            if (err) { throw err } else {
...
userModel.actualizarCelular = (info, callback) => {

    let sql = 'UPDATE usuarios SET telefono = ? WHERE id = ?;';
